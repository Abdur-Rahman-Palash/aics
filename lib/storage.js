...
class BaseStorage {
  ...
  // Agents
  async getAgents(businessId) { throw new Error('Not implemented'); }
  async addAgent(businessId, email) { throw new Error('Not implemented'); }
  async removeAgent(businessId, agentId) { throw new Error('Not implemented'); }
  
  // Products Catalog
  async getProducts(businessId) { throw new Error('Not implemented'); }
  async addProduct(businessId, productData) { throw new Error('Not implemented'); }
  async deleteProduct(businessId, productId) { throw new Error('Not implemented'); }
  async searchProducts(businessId, query, maxBudget) { throw new Error('Not implemented'); }
  ...
  async getWebsitesNeedRetraining() { throw new Error('Not implemented'); }
  ...
  async archiveConversations() { throw new Error('Not implemented'); }
  ...
  // Bookings
  async addBooking(businessId, bookingData) { throw new Error('Not implemented'); }
  async getBookings(businessId) { throw new Error('Not implemented'); }
  async updateBooking(businessId, bookingId, updates) { throw new Error('Not implemented'); }
  
  // Orders
  async addOrder(businessId, orderData) { throw new Error('Not implemented'); }
  async getOrder(businessId, orderId) { throw new Error('Not implemented'); }
  async getOrders(businessId) { throw new Error('Not implemented'); }
  async updateOrder(businessId, orderId, updates) { throw new Error('Not implemented'); }
  
  // Form Submissions
  async addFormSubmission(businessId, submissionData) { throw new Error('Not implemented'); }
  async getFormSubmissions(businessId) { throw new Error('Not implemented'); }
  
  // Knowledge Base
  async addKnowledgeBaseArticle(businessId, articleData) { throw new Error('Not implemented'); }
  async getKnowledgeBaseArticles(businessId) { throw new Error('Not implemented'); }
  async getKnowledgeBaseArticle(businessId, articleId) { throw new Error('Not implemented'); }
  async updateKnowledgeBaseArticle(businessId, articleId, updates) { throw new Error('Not implemented'); }
  async deleteKnowledgeBaseArticle(businessId, articleId) { throw new Error('Not implemented'); }
  
  // Translations
  async translateText(text, fromLang, toLang) { throw new Error('Not implemented'); }
}
...
class JSONStorage extends BaseStorage {
  ...
  async getBusinessesForUser(userId) {
    return this.data.businesses.filter(business => business.userId === userId || (business.agents && business.agents.includes(userId)));
  }

  async getBusiness(businessId, userId = null) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) return null;
    if (userId && business.userId !== userId && (!business.agents || !business.agents.includes(userId))) return null;
    return business;
  }

  async getAgents(businessId) {
    const business = await this.getBusiness(businessId);
    if (!business) return [];
    if (!business.agents) business.agents = [];
    return business.agents.map(agentId => {
      const u = this.data.users.find(user => user.id === agentId);
      return u ? { id: u.id, name: u.name, email: u.email } : null;
    }).filter(Boolean);
  }

  async addAgent(businessId, email) {
    const business = await this.getBusiness(businessId);
    if (!business) throw new Error('Business not found');
    const user = this.data.users.find(u => u.email === email);
    if (!user) throw new Error('User not found with this email');
    if (!business.agents) business.agents = [];
    if (business.agents.includes(user.id)) throw new Error('User is already an agent');
    if (business.userId === user.id) throw new Error('Owner cannot be added as an agent');
    business.agents.push(user.id);
    this.save();
    return { id: user.id, name: user.name, email: user.email };
  }

  async removeAgent(businessId, agentId) {
    const business = await this.getBusiness(businessId);
    if (!business) throw new Error('Business not found');
    if (!business.agents) return false;
    const index = business.agents.indexOf(agentId);
    if (index === -1) return false;
    business.agents.splice(index, 1);
    this.save();
    return true;
  }
  ...
  async getWebsitesNeedRetraining() {
    const needRetraining = [];
    const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
    
    for (const business of this.data.businesses) {
      const websites = business.knowledgeSources?.websites || [];
      for (const ws of websites) {
        if (ws.status === 'completed') {
          const lastTrained = ws.lastTrainedAt ? new Date(ws.lastTrainedAt) : null;
          if (!lastTrained || lastTrained < sevenDaysAgo) {
            needRetraining.push({
              businessId: business.id,
              qdrantCollection: business.qdrantCollection,
              websiteId: ws.id,
              url: ws.url
            });
          }
  ...
  async archiveConversations() {
    ...
    if (this.data && this.data.businesses) {
      this.data.businesses.forEach(business => {
        if (!business.conversations) return;
        business.conversations.forEach(conversation => {
          const updatedDate = new Date(conversation.updatedAt || conversation.createdAt || 0);
          if (updatedDate < thirtyDaysAgo && conversation.status !== 'archived') {
  ...
  async getProducts(businessId) {
    const business = await this.getBusiness(businessId);
    if (!business) return [];
    if (!business.products) business.products = [];
  ...
  async deleteProduct(businessId, productId) {
    ...
    if (!business.products) return false;
  ...
  async searchProducts(businessId, query, maxBudget = null) {
    ...
    return products.filter(p => {
      ...
      const matchesBudget = maxBudget === null || maxBudget === undefined || p.price <= Number(maxBudget);
  ...
  // Bookings
  async addBooking(businessId, bookingData) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) throw new Error('Business not found');
  ...
  async getBookings(businessId) {
    const business = this.data.businesses.find(b => b.id === businessId);
    return business ? (business.bookings || []) : [];
  }
  
  async updateBooking(businessId, bookingId, updates) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) return null;
  ...
  // Orders
  async addOrder(businessId, orderData) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) throw new Error('Business not found');
    
    if (!business.orders) business.orders = [];
  ...
  async getOrder(businessId, orderId) {
    ...
    return business.orders?.find(o => o.id === orderId || o.orderNumber === orderId);
  }
  
  async getOrders(businessId) {
    const business = this.data.businesses.find(b => b.id === businessId);
    return business ? (business.orders || []) : [];
  }
  
  async updateOrder(businessId, orderId, updates) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) return null;
    
    const order = business.orders?.find(o => o.id === orderId || o.orderNumber === orderId);
  ...
  // Form Submissions
  async addFormSubmission(businessId, submissionData) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) throw new Error('Business not found');
    
    if (!business.formSubmissions) business.formSubmissions = [];
    ...
    business.formSubmissions.push(submission);
    business.updatedAt = new Date().toISOString();
    this.save();
    return submission;
  }
  
  async getFormSubmissions(businessId) {
    const business = this.data.businesses.find(b => b.id === businessId);
    return business ? (business.formSubmissions || []) : [];
  }

  // Knowledge Base
  async addKnowledgeBaseArticle(businessId, articleData) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) throw new Error('Business not found');
    
    if (!business.knowledgeBase) business.knowledgeBase = [];
    ...
    business.knowledgeBase.push(article);
    business.updatedAt = new Date().toISOString();
    this.save();
    return article;
  }
  
  async getKnowledgeBaseArticles(businessId) {
    const business = this.data.businesses.find(b => b.id === businessId);
    return business ? (business.knowledgeBase || []) : [];
  }
  
  async getKnowledgeBaseArticle(businessId, articleId) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) return null;
    
    return business.knowledgeBase?.find(a => a.id === articleId);
  }
  
  async updateKnowledgeBaseArticle(businessId, articleId, updates) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) return null;
    
    const article = business.knowledgeBase?.find(a => a.id === articleId);
  ...
  async deleteKnowledgeBaseArticle(businessId, articleId) {
    const business = this.data.businesses.find(b => b.id === businessId);
    if (!business) return false;
    
    const index = business.knowledgeBase?.findIndex(a => a.id === articleId);
    if (index === -1) return false;
    
    business.knowledgeBase.splice(index, 1);
    business.updatedAt = new Date().toISOString();
    this.save();
    return true;
  }
  ...
  async translateText(text, fromLang, toLang) {
    // Simple translation placeholder - in production, use Google Cloud Translation or similar
    const translations = {
      'bn-en': {
        'হ্যালো': 'Hello',
        'কেমন আছ': 'How are you',
...
class NeonStorage extends BaseStorage {
  ...
  async createTables() {
    ...
    try {
      ...
      await client.query(`CREATE INDEX IF NOT EXISTS idx_business_agents_user_id ON business_agents(user_id)`);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_products_business_id ON products(business_id)`);
  ...
  async getBusinessesForUser(userId) {
    const result = await this.pool.query(
      `SELECT b.* FROM businesses b 
       LEFT JOIN business_agents ba ON b.id = ba.business_id 
       WHERE b.user_id = $1 OR ba.user_id = $1
       GROUP BY b.id`, 
      [userId]
    );
    ...
    // Get all FAQs, leads, knowledge sources, conversations, unanswered questions, canned responses, triggers, and products in single queries
    const [faqsResult, leadsResult, knowledgeSourcesResult, conversationsResult, unansweredQuestionsResult, cannedResponsesResult, triggersResult, productsResult] = await Promise.all([
        ...
        this.pool.query('SELECT * FROM products WHERE business_id = ANY($1) ORDER BY created_at DESC', [businessIds])
    ]);
    
    // Organize data by business ID
    const productsByBusiness = {};
    productsResult.rows.forEach(p => {
      if (!productsByBusiness[p.business_id]) productsByBusiness[p.business_id] = [];
      productsByBusiness[p.business_id].push({
        id: p.id,
    ...
    const businesses = result.rows.map(businessRow => ({
      ...
      products: productsByBusiness[businessRow.id] || [],
  ...
  async getBusiness(businessId, userId = null) {
    ...
    if (userId) {
      query = `
        SELECT b.* FROM businesses b
        LEFT JOIN business_agents ba ON b.id = ba.business_id
        WHERE b.id = $1 AND (b.user_id = $2 OR ba.user_id = $2)
        GROUP BY b.id
      `;
      params.push(userId);
    }
    ...
    business.userId = business.user_id;
    ...
    const [faqs, leads, knowledgeSources, conversations, unansweredQuestions, cannedResponses, triggers, products] = await Promise.all([
  ...
  async updateWidgetSettings(businessId, settings) {
    const check = await this.pool.query('SELECT widget_settings FROM businesses WHERE id = $1', [businessId]);
    const current = check.rows[0]?.widget_settings || {};
  ...
  async getWebsitesNeedRetraining() {
    const query = `
      SELECT ks.id as website_id, ks.url, b.id as business_id, b.qdrant_collection
      FROM knowledge_sources ks
      JOIN businesses b ON ks.business_id = b.id
      WHERE ks.type = 'website' AND ks.status = 'completed' AND (ks.last_trained_at IS NULL OR ks.last_trained_at < NOW() - INTERVAL '7 days')
    `;
  ...
  async archiveConversations() {
    ...
    const getResult = await this.pool.query(
      "SELECT id, business_id, messages FROM conversations WHERE (updated_at < $1 OR (updated_at IS NULL AND created_at < $1)) AND status != 'archived'",
      [thirtyDaysAgo]
    );
    ...
    for (const row of getResult.rows) {
      ...
      await this.pool.query(
        "UPDATE conversations SET status = 'archived', messages = $1, updated_at = NOW() WHERE id = $2 AND business_id = $3",
        [JSON.stringify(archivedMessages), row.id, row.business_id]
      );
      count++;
    }
  ...
  async getAgents(businessId) {
    const result = await this.pool.query(
      `SELECT u.id, u.name, u.email, ba.role 
  ...
  async addAgent(businessId, email) {
    const userResult = await this.pool.query('SELECT id, name, email FROM users WHERE email = $1', [email]);
    if (userResult.rows.length === 0) throw new Error('User not found with this email');
    const user = userResult.rows[0];

    const businessResult = await this.pool.query('SELECT user_id FROM businesses WHERE id = $1', [businessId]);
    if (businessResult.rows.length === 0) throw new Error('Business not found');
    if (businessResult.rows[0].user_id === user.id) throw new Error('Owner cannot be added as an agent');

    await this.pool.query(
      'INSERT INTO business_agents (business_id, user_id, role) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING',
      [businessId, user.id, 'agent']
    );
    return { id: user.id, name: user.name, email: user.email };
  }
  ...
  async removeAgent(businessId, agentId) {
    const result = await this.pool.query(
      'DELETE FROM business_agents WHERE business_id = $1 AND user_id = $2',
      [businessId, agentId]
    );
    return result.rowCount > 0;
  }
  ...
  async addProduct(businessId, productData) {
    const id = require('crypto').randomBytes(16).toString('hex');
    const sanitizedProduct = sanitizeObject(productData);
    const result = await this.pool.query(
      'INSERT INTO products (id, business_id, name, description, price, image_url, link_url, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW()) RETURNING *',
      [
        id,
        businessId,
        sanitizedProduct.name,
        sanitizedProduct.description || '',
        Number(sanitizedProduct.price) || 0,
        sanitizedProduct.imageUrl || '',
        sanitizedProduct.linkUrl || ''
      ]
    );
  ...
  async searchProducts(businessId, query, maxBudget = null) {
    const lowerQuery = `%${(query || '').toLowerCase()}%`;
    let sql = 'SELECT * FROM products WHERE business_id = $1 AND (LOWER(name) LIKE $2 OR LOWER(description) LIKE $2)';
    const params = [businessId, lowerQuery];
...
async function getStorage() {
  if (process.env.DATABASE_URL) {
    ...
    try {
      ...
      console.error('Failed to initialize Neon storage:', error.message || error);
      if (storage.pool) {
        try {
          await storage.pool.end();
        } catch (closeError) {
          console.error('Error closing Neon connection pool after failed init:', closeError.message || closeError);
        }
      }
      console.warn('Falling back to JSON file storage for local development.');
