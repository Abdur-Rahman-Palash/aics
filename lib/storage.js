...
class NeonStorage extends BaseStorage {
  ...
  async getBusinessesForUser(userId) {
    ...
    // Get all FAQs, leads, knowledge sources, conversations, and unanswered questions in single queries
    const [faqsResult, leadsResult, knowledgeSourcesResult, conversationsResult, unansweredQuestionsResult] = await Promise.all([
  ...
  async getBusiness(businessId, userId = null) {
    ...
    const [faqs, leads, knowledgeSources, conversations, unansweredQuestions] = await Promise.all([
