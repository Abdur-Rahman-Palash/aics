...
4. Keep it friendly and helpful`;
  ...
  async classifyQuestionRelevance(userQuestion, contextChunks) {
    ...
    const systemPrompt = `Determine if the user's question is related to the website's purpose, even if the exact answer isn't in the context.
- If the question is about tasks the website is designed for (like creating invoices, managing payments, etc. based on the website knowledge), answer RELATED
- Only answer UNRELATED if the question is completely off-topic (like sports scores, weather, etc.)

Answer ONLY: RELATED or UNRELATED.
...
`;
    ...
    return text === 'RELATED' || text.includes('RELATED');
  }
}
